...
6. **Graceful capability detection.** The plugin can connect to different CannaAI deployments and reports which features the connected backend actually supports.
...
ChatGPT must be able to determine which CannaAI capabilities are currently reachable before relying on specialized tools.
...
**Registration rule:** read-only parity tools are registered consistently so ChatGPT has a stable tool surface. If the connected backend does not support a capability, the tool returns `CANNAAI_UNSUPPORTED` with a concise explanation. Consequential write tools are different: they are only registered when the corresponding server-side feature flags are enabled.

This avoids a tool list that unpredictably changes after transient backend discovery failures while still preventing write capabilities from being exposed by default.
...
- write tools remain unregistered unless `CANNAAI_ENABLE_WRITE_TOOLS=true`.
- physical automation tools remain unregistered unless both `CANNAAI_ENABLE_WRITE_TOOLS=true` and `CANNAAI_ENABLE_AUTOMATION=true`.
- enabling consequential tools requires a non-empty `CANNAAI_ACTION_SIGNING_SECRET`.
...
`actionId` is a short-lived, URL-safe signed ticket generated by `src/lib/action-ticket.js`. The ticket binds:

- action name
- target identifiers
- requested change hash
- current-state hash or backend revision identifier
- issued-at timestamp
- expiry timestamp
- random nonce

The ticket is authenticated with HMAC-SHA-256 using `CANNAAI_ACTION_SIGNING_SECRET`. The secret is never included in the ticket or returned to the client.

This design is stateless across plugin requests and works across multiple plugin replicas as long as they share the same signing secret.
...
- the action ticket signature is invalid
- the preview expired
- target state materially changed since preview
- the requested action differs from the signed preview
- required backend authorization is unavailable

The execute handler re-reads the target state before mutation and compares it to the state hash or backend revision bound into the signed ticket.
...
10. A strong action-signing secret when consequential tools are enabled.
...
- normalized errors
- schema validation
- HMAC action-ticket creation and verification
...
Read-only parity tools remain registered even when the backend lacks a capability; unsupported calls return `CANNAAI_UNSUPPORTED`. Consequential tools must not be registered when their feature flags are disabled.

### 11.4 Action safety tests

Verify that:

- mutation tools are absent by default
- preview never mutates state
- action tickets cannot be modified without invalidating the HMAC
- execute requires an unexpired matching action ticket
...
The parity project is intentionally split into independently testable stages. **Each stage receives its own implementation plan and test/commit cycle** rather than one enormous all-at-once plan.
...
8. Consequential action tickets are signed, short-lived, bound to the requested change and observed state, and revalidated immediately before execution.
9. The public repository contains no real credentials or private grow data.
10. Automated tests cover client behavior, MCP contracts, unsupported capabilities, action-ticket integrity, and action safeguards.
11. The README documents mock mode, real CannaAI mode, configuration, security expectations, and the supported parity matrix.
